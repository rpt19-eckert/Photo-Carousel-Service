...
class PhotoService extends React.Component {
  ...
  componentDidMount() {
    ...
    fetch(`http://localhost:3002/listing-info/${id}`)
      .then((res) => {
        if (!res.ok) {
      ...
      this.setState((prevState) => {
        return {
          nextPrevBorders: ["none", "none", "none", "none"].map((x, i) => {
            if (
              i ===
                prevState.nextPrevBorders.indexOf("2px solid #404040") - 1 ||
              (prevState.nextPrevBorders.indexOf("2px solid #404040") === 0 &&
                i === 0)
            ) {
          ...
          nextPrevOpacities: ["70%", "70%", "70%", "70%"].map((x, i) => {
            if (
              i === prevState.nextPrevOpacities.indexOf("100%") - 1 ||
              (prevState.nextPrevOpacities.indexOf("100%") === 0 && i === 0)
            ) {
              return "100%";
            } else {
              return x;
        ...
        return {
          nextPrevBorders: ["none", "none", "none", "none"].map((x, i) => {
            if (
              i ===
                prevState.nextPrevBorders.indexOf("2px solid #404040") + 1 ||
              (prevState.nextPrevBorders.indexOf("2px solid #404040") === 3 &&
                i === 3)
            ) {
              return "2px solid #404040";
            } else {
          ...
          nextPrevOpacities: ["70%", "70%", "70%", "70%"].map((x, i) => {
            if (
              i === prevState.nextPrevOpacities.indexOf("100%") + 1 ||
              (prevState.nextPrevOpacities.indexOf("100%") === 3 && i === 3)
            ) {
              return "100%";
            } else {
              return x;
